test_that("HCP parity fixtures match corr, multreg, and combined outputs", {
  ...
  r2_ref_metric <- read_actflower_h5(fixture, "metrics_corr/R2_vals")
  mae_ref_metric <- read_actflower_h5(fixture, "metrics_corr/mae_vals")
  multreg_ref_metric <- read_actflower_h5(fixture, "metrics_multreg/corr_vals")
  multreg_r2_ref_metric <- read_actflower_h5(fixture, "metrics_multreg/R2_vals")
  multreg_mae_ref_metric <- read_actflower_h5(fixture, "metrics_multreg/mae_vals")
  combined_ref_metric <- read_actflower_h5(fixture, "metrics_combined/corr_vals")
  combined_r2_ref_metric <- read_actflower_h5(fixture, "metrics_combined/R2_vals")
  combined_mae_ref_metric <- read_actflower_h5(fixture, "metrics_combined/mae_vals")

  expect_lt(max(abs(out_corr$model_compare_output$corr_vals - corr_ref_metric)), 1e-8)
  expect_lt(max(abs(out_corr$model_compare_output$R2_vals - r2_ref_metric)), 1e-8)
  expect_lt(max(abs(out_corr$model_compare_output$mae_vals - mae_ref_metric)), 1e-8)
  expect_lt(max(abs(out_multreg$model_compare_output$corr_vals - multreg_ref_metric)), 1e-5)
  expect_lt(max(abs(out_multreg$model_compare_output$R2_vals - multreg_r2_ref_metric)), 1e-5)
  expect_lt(max(abs(out_multreg$model_compare_output$mae_vals - multreg_mae_ref_metric)), 1e-5)
  expect_lt(max(abs(out_combined$model_compare_output$corr_vals - combined_ref_metric)), 1e-5)
  expect_lt(max(abs(out_combined$model_compare_output$R2_vals - combined_r2_ref_metric)), 1e-5)
  expect_lt(max(abs(out_combined$model_compare_output$mae_vals - combined_mae_ref_metric)), 1e-5)
})
